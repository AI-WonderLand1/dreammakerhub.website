...
CREATE OR REPLACE FUNCTION public.reserve_coder_workspace_slot(
  p_user_id uuid, p_workspace_name text, p_limit integer
) RETURNS uuid
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.coder_workspace_slots
    ...
    RETURN NULL;
  END IF;
  INSERT INTO public.coder_workspace_slots(user_id, workspace_name)
    VALUES (p_user_id, p_workspace_name) RETURNING id INTO v_slot;
  RETURN v_slot;
END;
$$;
REVOKE ALL ON FUNCTION public.reserve_coder_workspace_slot(uuid, text, integer) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reserve_coder_workspace_slot(uuid, text, integer) TO service_role;
