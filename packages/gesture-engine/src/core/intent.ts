...
export interface MotionSample {
  ...
  wrist: { x: number; y: number; z: number }  // normalized 0-1 frame coords
  velocity: number           // normalized units/sec, computed from previous sample
  direction: number          // radians, computed from previous sample
}

export interface IntentResult {
  isIntentional: boolean
  confidence: number         // 0-1
  reason: string             // for debugging/logging — not shown to user
}
...
export class IntentDetector {
  ...
  evaluate(): IntentResult {
    ...
    const inZone =
      latest.wrist.x >= INTERACTION_ZONE.minX &&
    ...
    const avgVelocity =
      this.buffer.reduce((sum, s) => sum + s.velocity, 0) / this.buffer.length
