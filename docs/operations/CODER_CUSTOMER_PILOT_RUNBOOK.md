...
Never infer any of these values from a screenshot, table count, passing unit tests, or a Coder health badge. When the pilot is enabled, the current WonderSpace page displays the customer form for non-admins while leaving the operator flow intact. It intentionally does not yet provide a working in-site IDE gateway.
