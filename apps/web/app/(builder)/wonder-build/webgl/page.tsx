...
function TemplateGallery() {
  ...
  useEffect(() => {
    if (sceneIdParam) {
      router.replace(`/wonder-build/webgl/editor/${sceneIdParam}`);
    } else if (sceneParam) {
      router.replace(`/wonder-build/webgl/editor/${sceneParam}`);
    }
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold">WebGL Scene Templates</h1>
            <p className="mt-1 text-sm text-white/50">Pick a template to start editing in WebGL Studio</p>
          </div>
          <Link
            href="/wonder-build/webgl/editor/blank_canvas"
...
export default function WebglGalleryPage() {
  return (
