...
export const metadata = {
  title: 'Blog | DreamMakerHub',
  description: 'DreamMakerHub product updates, founder notes, development lessons, and tutorials.',
};
    ...
    <main className="min-h-screen bg-[#050508] text-slate-200">
      <section className="mx-auto max-w-6xl px-5 py-16 md:px-8">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-base leading-7 text-zinc-400 md:text-lg">Product updates, technical lessons, founder notes, and the messy parts of building DreamMakerHub. Anyone can read. No account required.</p>
        </div>

        <div className="mt-12 grid gap-5">
          {posts.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group rounded-2xl border border-white/10 bg-zinc-950/50 p-6 transition hover:border-violet-500/30 hover:bg-zinc-950">
              ...
              <div className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-violet-300">Read article <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></div>
            </Link>
          ))}
        </div>

        <div className="mt-10 border-t border-white/10 pt-6 text-sm text-zinc-500">
          Looking for Q&A or support instead? <Link href="/community" className="text-violet-300 hover:text-violet-200">Visit the community.</Link>
        </div>
      </section>
    </main>
  );
