          ...
          {plans.map((plan: Plan) => (
            <div
              ...
              className={`relative flex flex-col rounded-2xl border p-5 transition sm:p-6 ${
                plan.highlight
                  ? "border-purple-500/60 bg-gradient-to-b from-purple-900/30 to-purple-950/20 shadow-xl shadow-purple-900/20 lg:scale-[1.03]"
                  : plan.id === "team"
            ...
            >
              {plan.highlight && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-gradient-to-r from-pink-500 to-purple-600 px-3 py-0.5 text-[10px] font-bold text-white shadow-md">
                  MOST POPULAR
                </span>
              )}
              ...
              <p className="mb-5 text-[13px] leading-5 text-gray-400">{plan.desc}</p>
              <ul className="mb-6 flex-1 space-y-2.5">
                {plan.bullets.map((b: string) => (
                  <li key={b} className="flex items-start gap-2 text-[13px] leading-5 text-gray-300">
