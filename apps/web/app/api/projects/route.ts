...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, message: 'Failed to list projects' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes('PROJECT_LIMIT_REACHED')) {
      return NextResponse.json({ ok: false, code: 'PROJECT_LIMIT', message: 'Project limit reached for your plan.' }, { status: 429 });
    }
