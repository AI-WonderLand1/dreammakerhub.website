...
export const notificationRenderers: Record<string, BlockRenderer> = {
  'toast': ({ el, selectedId, selectElement, baseProps, children }) => {

  },
  'banner': ({ el, selectedId, selectElement, baseProps, children }) => {

  },
  'announcement-bar': ({ el, selectedId, selectElement, baseProps, children }) => {
      return <div {...baseProps} className="text-center text-xs font-medium px-3 py-1.5">{el.props.message || el.props.text}{children}</div>;
  },
  'push-notification': ({ el, selectedId, selectElement, baseProps, children }) => {
  ...
  'live-alert': ({ el, selectedId, selectElement, baseProps, children }) => {
