...
export const formsRenderers: Record<string, BlockRenderer> = {
  ...
  'input': ({ el, selectedId, selectElement, baseProps, style, children }) => {
      return <div {...baseProps}><label className="block text-xs text-white/50 mb-1">{el.props.label}</label><input aria-label={el.props.label || el.props.placeholder || 'Input'} type={el.props.type || 'text'} placeholder={el.props.placeholder} className="w-full rounded border border-white/10 bg-black/40 px-2 py-1 text-xs text-white" /></div>;
  },
  'checkbox': ({ el, selectedId, selectElement, baseProps, style, children }) => {
      return <div {...baseProps} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><input aria-label={el.props.label || 'Checkbox'} type="checkbox" checked={el.props.checked || false} readOnly /><span className="text-xs text-white/70">{el.props.label}</span>{children}</div>;
  },
  ...
  'search': ({ el, selectedId, selectElement, baseProps, style, children }) => {
      return <div {...baseProps} style={{ display: 'flex', gap: '0.5rem' }}><input aria-label="Search" type="text" placeholder={el.props.placeholder || 'Search...'} className="flex-1 rounded border border-white/10 bg-black/40 px-2 py-1 text-xs text-white" /><button className="rounded bg-purple-600 text-white px-2 text-xs">{el.props.buttonText || '🔍'}</button>{children}</div>;
  },
  'product-search': ({ el, selectedId, selectElement, baseProps, style, children }) => {
      return <div {...baseProps} style={{ display: 'flex', gap: '0.5rem' }}><input aria-label="Search products" type="text" placeholder={el.props.placeholder || 'Search...'} className="flex-1 rounded border border-white/10 bg-black/40 px-2 py-1 text-xs text-white" /><button className="rounded bg-purple-600 text-white px-2 text-xs">{el.props.buttonText || '🔍'}</button>{children}</div>;
  },
    ...
    <div {...baseProps} style={style}>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', justifyContent: 'center', margin: '0.75rem 0' }}>
        {Array.from({ length: 11 }, (_, i) => (
          <button key={i} type="button" aria-label={`Score ${i} out of 10`} style={{ width: 34, height: 34, borderRadius: 6, border: '1px solid rgba(255,255,255,0.18)', background: i <= 6 ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.15)', color: '#fff', cursor: 'pointer' }}>{i}</button>
        ))}
      </div>
  ...
  'login-form': ({ el, baseProps, style, children }) => (
    <form {...baseProps} style={style} onSubmit={(e: any) => e.preventDefault()}>
      ...
      <input aria-label="Email" type="email" placeholder={el.props.emailPlaceholder || 'Email'} required style={{ padding: '0.6rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
      <input aria-label="Password" type="password" placeholder="Password" required style={{ padding: '0.6rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
  ...
  'register-form': ({ el, baseProps, style }) => (
    <form {...baseProps} style={style} onSubmit={(e: any) => e.preventDefault()}>
      <h3 style={{ margin: 0 }}>{el.props.heading}</h3>
      <input aria-label="Email" type="email" placeholder="Email" required style={{ padding: '0.6rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
      <input aria-label="Password" type="password" placeholder="Password" required minLength={8} title={el.props.passwordHint} style={{ padding: '0.6rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
  ...
  'otp-input': ({ el, baseProps, style }) => (
    <div {...baseProps} style={style} role="group" aria-label="One-time code">
      {Array.from({ length: el.props.digits || 6 }, (_, i) => (
        <input key={i} aria-label={`Digit ${i + 1} of ${el.props.digits || 6}`} maxLength={1} inputMode="numeric" type={el.props.masked ? 'password' : 'text'} style={{ width: 42, height: 50, textAlign: 'center', fontSize: '1.2rem', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
      ))}
    ...
    <form {...baseProps} style={style} onSubmit={(e: any) => e.preventDefault()}>
      ...
      <div style={{ flex: 1, display: 'grid', gap: '0.5rem' }}>
        {(el.props.fields || []).map((f: any) => (
          <input key={f.key} aria-label={f.label || f.key || 'Profile field'} placeholder={f.label} style={{ padding: '0.5rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.25)', color: '#fff' }} />
        ))}
    ...
    <aside {...baseProps} style={style} aria-label="Filters">
      {(el.props.filters || []).map((f: any) => (
        <fieldset key={f.label} style={{ border: 0, padding: 0, margin: 0 }}>
          ...
          {f.type === 'range' && <input aria-label={`${f.label || 'Filter'} range`} type="range" style={{ width: '100%' }} />}
  ...
  'rating-feedback': ({ el, baseProps, style }) => (
    <div {...baseProps} style={style}>
      ...
      {el.props.askComment && <textarea aria-label={el.props.commentPlaceholder || 'Feedback comment'} placeholder={el.props.commentPlaceholder} rows={2} style={{ width: '100%', marginTop: '0.6rem', padding: '0.5rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.25)', color: '#fff' }} />}
  ...
  'newsletter-inline': ({ el, baseProps, style, children }) => (
    <form {...baseProps} style={style} onSubmit={(e: any) => e.preventDefault()}>
      <input aria-label="Email address" type="email" placeholder={el.props.placeholder} required style={{ flex: 1, minWidth: 180, padding: '0.55rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff' }} />
