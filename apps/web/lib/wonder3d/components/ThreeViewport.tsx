...
export const ThreeViewport: React.FC<ThreeViewportProps> = ({
  ...
  useEffect(() => {
    ...
    // 6. Build & Add Procedural 3D Model
  ...
  // Update Lighting Rig
  const setupLighting = (scene: THREE.Scene, type: ViewportLighting) => {
    ...
    switch (type) {
      case 'cyberpunk': {
        ...
        const light1 = new THREE.DirectionalLight(0x00f0ff, 3.5);
        light1.position.set(5, 5, 5);
        scene.add(light1);

        const light2 = new THREE.DirectionalLight(0xff007f, 3.0);
        light2.position.set(-5, -2, -5);
        scene.add(light2);
        break;
      }
      case 'sunset': {
        scene.add(new THREE.AmbientLight(0x221111, 1.0));
        const keyLight = new THREE.DirectionalLight(0xff7700, 3.8);
        keyLight.position.set(6, 4, 3);
        scene.add(keyLight);

        const fillLight = new THREE.DirectionalLight(0x2266aa, 1.5);
        fillLight.position.set(-5, 2, -4);
        scene.add(fillLight);
        break;
      }
      case 'void': {
        scene.add(new THREE.AmbientLight(0x050505, 0.5));
        const spotLight = new THREE.SpotLight(0xffffff, 5.0, 20, Math.PI / 4, 0.5);
