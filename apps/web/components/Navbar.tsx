...
const LINKS = [
  ...
  { label: 'NPC-AI-SIM', href: '/wonder-play', icon: Bot },
  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#05070c]/90 shadow-[0_12px_40px_rgba(0,0,0,.28)] backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" onClick={() => setMobileOpen(false)} className="group flex min-w-0 shrink-0 items-center gap-3" aria-label="DreamMakerHub home">
          ...
          {LINKS.map(({ label, href, external }) => {
            ...
            return (
              <Link
                ...
                className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${active ? 'bg-violet-500/15 text-white ring-1 ring-violet-400/20' : 'text-white/60 hover:bg-white/[0.055] hover:text-white'}`}
              >
                {label}
              </Link>
            );
        ...
        <div className="ml-auto hidden shrink-0 items-center gap-2 sm:flex">
          {!loading && user ? (
            <>
              <Link href="/dashboard" className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.055] px-3.5 text-sm font-semibold text-white/80 transition hover:border-violet-400/30 hover:bg-white/[0.09] hover:text-white">
                <LayoutDashboard size={15} /> Dashboard
              </Link>
              <button type="button" onClick={() => void handleSignOut()} disabled={signingOut} className="inline-flex h-10 items-center gap-2 rounded-xl px-3 text-sm font-semibold text-white/45 transition hover:bg-red-500/10 hover:text-red-200 disabled:opacity-50">
                <LogOut size={15} /> {signingOut ? 'Signing out…' : 'Sign out'}
              </button>
            </>
          ) : !loading ? (
            <>
              <Link href="/public-pages/auth" className="inline-flex h-10 items-center rounded-xl px-3.5 text-sm font-semibold text-white/65 transition hover:bg-white/[0.055] hover:text-white">Sign in</Link>
              <Link href="/public-pages/auth?signup=true" className="inline-flex h-10 items-center rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 px-4 text-sm font-bold text-white shadow-lg shadow-violet-950/30 transition hover:brightness-110">Start free</Link>
            </>
          ) : (
            ...
            {LINKS.map(({ label, href, icon: Icon, external }) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${active ? 'bg-violet-500/15 text-white' : 'text-white/65 hover:bg-white/[0.055] hover:text-white'}`}
                >
            ...
            <div className="mt-3 border-t border-white/10 pt-3 sm:hidden">
              {!loading && user ? (
                <div className="grid gap-2">
                  <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 px-4 py-3 text-sm font-bold text-white">
                    <LayoutDashboard size={16} /> Open Dashboard
                  </Link>
                  <button type="button" onClick={() => void handleSignOut()} disabled={signingOut} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-red-200/75 hover:bg-red-500/10 disabled:opacity-50">
