...
import type { WebglHostProps } from "@/components/WebglEditorHost";
...
export function DirectWebglHost({ sceneId, onReady, onError, onStatus, onSceneChange }: WebglHostProps) {
  ...
  useEffect(() => {
    ...
    async function bootstrapAndMount() {
      ...
      try {
        ...
        if (cancelled) return;
        const bootstrapError = error instanceof Error ? error : new Error("WebGL Studio bootstrap script failed to load");
      ...
      if (!container) {
        const containerError = new Error("WebGL Studio direct container not found");
      ...
      if (!bootstrap) {
        const bootstrapApiError = new Error("WebGL Studio bootstrap finished but API is unavailable");
      ...
      try {
        ...
        if (!cleanup || typeof cleanup.destroy !== "function") {
          logger.warn("WebGL Studio mount did not return a cleanup object with destroy method. Memory leaks may occur.");
        }
        ...
        if (cancelled) return;
        const mountError = error instanceof Error ? error : new Error("WebGL Studio direct mount failed");
  ...
  return (
    <div className="relative h-full w-full">
      <div ref={containerRef} className="h-full w-full" data-testid="webgl-direct-host" />
      {failure ? (
        <div className="absolute inset-4 flex flex-col items-center justify-center rounded-xl border border-red-400/50 bg-red-900/80 p-4 text-center text-white">
          <p className="text-sm font-semibold">WebGL Studio direct bootstrap failed</p>
